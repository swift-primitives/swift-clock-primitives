...
extension Clock.Continuous.Deadline: Comparable {
    /// Orders two deadlines by their expiry instant.
    @inlinable
...
extension Clock.Continuous.Deadline {
    ...
    /// - Returns: `true` if `instant` is at or past the deadline.
    @inlinable
    ...
    /// - Returns: The duration from `instant` to the deadline, or `.zero` if expired.
    @inlinable
